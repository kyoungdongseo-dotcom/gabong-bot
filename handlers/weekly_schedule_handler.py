...
def parse_date(text: str, default_year: int = None) -> datetime | None:
    ...
    patterns = [
        ...
        (r'^(\d{1,2})$', lambda m: datetime(year, 5, int(m[0]))),
    ]
...
def parse_schedule_from_rows(rows):
    ...
    date_col = config.get('weekly_schedule_date_col', 0)
    region_col = config.get('weekly_schedule_region_col', 1)
    program_col = config.get('weekly_schedule_program_col', 2)
    ...
    for row in rows:
        ...
        if dt and week_start <= dt.date() <= week_end:
            date_key = dt.date()
            region = row[region_col] if len(row) > region_col else ''
            program = row[program_col] if len(row) > program_col else ''
            
            if not day_events.get(date_key):
                day_events[date_key] = []
            if region or program:
                day_events[date_key].append(f"{region}: {program}".strip(': '))
